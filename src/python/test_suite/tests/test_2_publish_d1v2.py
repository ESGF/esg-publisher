...
class Test2VerifyPublishSingleDatasetTwoVersions(test_1_publish_d1v1.Test1VerifyPublishSingleDatasetSingleVersion):
    ...
    def test_201_put_files_on_disk_d1v2(self):
        self.tlog("Putting files on disk: %s" % ds2.id)
        publisher.put_files_on_disk(ds2.id, ds2.files)

    def test_202_verify_files_on_disk_d1v2(self):
        self.tlog("Verifying files on disk: %s" % ds2.id)
        verify.verify_files_on_disk(ds2.id, ds2.files)

    def test_203_publish_to_db_d1v2(self):
        self.tlog("Publishing to db: %s" % ds2.id)
        publisher.publish_to_db(ds2.id, ds2.files)

    def test_204_verify_published_to_db_d1v2(self):
        self.tlog("Verifying files on disk: %s" % ds2.id)
